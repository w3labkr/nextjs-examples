...
function getLocale(request: NextRequest): string | undefined | null {
  const { pathname } = request.nextUrl;
  let locale: string | undefined | null;

  locale = languages.find((locale: string) => pathname.startsWith(`/${locale}/`) || pathname === `/${locale}`);
...
export function middleware(request: NextRequest) {
  ...
  const pathnameHasSlug = (slug: string): boolean =>
